...
modules_list="funct_summary_view or funct_detailed_view_filter or funct_analytics_view_filter or funct_heat_map_cie"
report_file="report_full/aiops_regression_cra_$name.html"
email_recipient="aneelkumar.ammanagi@ibm.com"
pytest -m "$modules_list" --html=$report_file --self-contained-html -v

echo "##### uploading the log to git #####"
if [ -f "$report_file" ]; then
  git config --global user.name "Aneelkumar-Ammanagi"
  git config --global user.email "aneelkumar.ammanagi@ibm.com" # replace with actual email
  git add "$report_file"
